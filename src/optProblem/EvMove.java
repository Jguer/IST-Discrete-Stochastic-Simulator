...
/**
 * @author antonio
 * This subclass is an extension of the abstract Event class and will handle the Move events.
...
public class EvMove extends Event{
	 ...
	 * In this particular case we move the individual in the event to one of the valid adjacent points.
	...
	public void ExecEvent(OptProblem opp) {
		...
		while(op.map.nonValidMove(actPoint,index,op.map.obstacles)) {
			index = r.nextInt(4); //can be improved probably. usually is pretty fast but sometimes can take a while to get a good index so yeah
		}
		...
		if( index != -1) { //if this point closes a cycle --> delete the cycle and remove those costs
			...
			//if we are going to remove a cycle that contains the goal than this individual will no longer have hit the goal
			if(Point.findSamePoint(subPoints,op.goal) != -1)
				ind.hit = false;
			...
			//delete these lists and the points/costs in the ind.history ones that have the same place in memory
			subPoints.clear(); 
			subCosts.clear();
			ind.cost -=cycle_cost;

...
public class Map implements IMap {
	...
	/**
	 * Method that lets us know if a move is valid or not. 
	 * If the move is not valid then we return true, false is returned otherwise.
	 * Quick reminder that 0 = move up, 1 = move right, 2 = move down, 3 = move left
	...
	public boolean nonValidMove(Point actPoint,int index,Point[] obstacles) {
		...
		if(index == 0 && Arrays.asList(obstacles).contains(new Point(actPoint.x,actPoint.y+1))) return true;
		if(index == 1 && Arrays.asList(obstacles).contains(new Point(actPoint.x+1,actPoint.y))) return true;
		if(index == 2 && Arrays.asList(obstacles).contains(new Point(actPoint.x,actPoint.y-1))) return true;
		if(index == 3 && Arrays.asList(obstacles).contains(new Point(actPoint.x-1,actPoint.y))) return true;
	...
	/**
	 * This method is a static method since it is not invoked from any Map object. It gets a pair of adjacent points and returns the cost associated with the edge between them.
	 * @param actPoint is the point in which the Individual is right now.
	...
	public int getCost(Point actPoint, Point newPoint, List<SpecialZone>specialZones) {
		...
		for(SpecialZone ed: specialZones){
			...
			y1 = ed.p1.y;
			y2 = ed.p2.y;
			// if we are coming from inside the zone to another point inside the zone we want to count the limits
			if((ax>=x1 && ax<=x2) && (ay>=y1 && ay<=y2) && (nx>=x1 && nx<=x2) && (ny>=y1 && ny<=y2)){
				return ed.cost;
			}
			// if we are coming from OUTSIDE the zone to a point inside the zone we DONT want to count the limits
